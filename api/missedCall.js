...
export default async function handler(req, res) {
  ...
  const { data: lead } = await supabase
    ...
    .eq('id', lead_id)
    .single();
  if (!lead) return res.status(404).json({ error: 'Лийдът не е намерен' });

  // 2️⃣ Determine message type based on previous messages
  ...
  switch (messages.length) {
    case 0:
      ...
      break;
    case 1:
      messageType = 'последващо_1';
      break;
    case 2:
      messageType = 'последващо_2';
      break;
    default:
      return res.json({ status: 'Няма повече последващи съобщения' });
  }

  // 3️⃣ Get clinic name from settings
  ...
  await supabase
    .from('leads')
    .update({ status: 'контактиран', last_message_id: newMessage.id })
    .eq('id', lead_id);
